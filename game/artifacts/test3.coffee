class Artifact.Test3 extends Artifact
  constructor: ->
    super
    @createSlider  150
    @createSlider -150
...
class Knob extends Artifact.SubObject
  ...
  onTouch:  ->
    console.log 'TODO: make these things draggable'
    
    return if @animator?.animating
    if @slot == 0
      @slot = if @goingUp then 1 else -1
      @goingUp = !@goingUp
    ...
    startZ = @position.z
    @animator = new Animator 0.5, (progress) =>
      @position.z = Math.between(startZ, 150 * @slot, progress)
    
    @animator.complete = => @artifact.checkWinButton()